...
if ($id==38) {
	...
	if (!empty($telefono)) {
	   if (!is_numeric($telefono)) {
		...
		mysqli_query($conexion, "UPDATE usuario SET nombres = '$nombres', apellidos = '$apellidos',sexo='$sexo',telefono='$telefono',correo='$correo',usuario='$usuario',clave='$combinado', direccion = '$direccion', descripcion='$descripcion', tipo = '$tipo',estado = '$estado' WHERE id_usuario = '$id'");
	
	$error= "<script>$('.mensaje').addClass('bien').html('Edicion completado con exito').show(200).delay(2500).hide(200);</script>";
	}
}else{

	mysqli_query($conexion, "UPDATE cuenta SET nombres = '$nombres', apellidos = '$apellidos', sexo='$sexo', telefono='$telefono',correo='$correo',usuario='$usuario',clave='$combinado', direccion = '$direccion', descripcion='$descripcion', tipo = '$tipo',estado = '$estado' WHERE id_usuario = '$id'");
